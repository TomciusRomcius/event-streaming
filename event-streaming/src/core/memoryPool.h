...
{
public:
    MemoryChunkUser(MemoryChunk& memoryChunk) : m_MemoryChunk(memoryChunk)
    {
...
public:
    ...
    std::optional<MemoryChunkUser> GetMemoryChunk(int16_t chunkSize)
    {
        ...
        for (auto it = m_MemoryChunks.begin(); it != m_MemoryChunks.end(); ++it)
        {
            ...
            {
                // Its okay to remove const cast, since size won't be changed. TODO:
        ...
        // Its okay to remove const cast, since size won't be changed. TODO: should
