...
void EventSystem::Subscribe(std::string& eventType, GroupId groupId, SocketType socket)
{
    ...
    } else
    {
        ...
        auto groupIt = std::find_if(groups.begin(), groups.end(),
                                    [groupId](EventGroup& group) { return group.EventGroupId == groupId; });
...
void EventSystem::Unsubscribe(std::string& eventType, GroupId groupId, SocketType socket)
{
    ...
    {
        LOG_ERROR("Failed unsubscribe socket {} from event type '{}' and group "
                  "'{}': event type or group does not exist",
                  socket, groupId, eventType);
        return;
    }
